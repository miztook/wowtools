...
public:
	...
	string_ncs(const char* c, uint32_t len)
	{
		if (len >= MaxSize)
			len = MaxSize-1;
	...
	string_ncs(const string_ncs& other)
	{
	...
	string_ncs& operator=(const char* c)
	{
		Q_strcpy(data, MaxSize, c);
		used = (uint32_t)strlen(data);
		data[used] = '\0';
	...
	string_ncs& operator=(const string_ncs& other)
	{
		Q_strcpy(data, MaxSize, other.data);
		used = (uint32_t)strlen(data);
		data[used] = '\0';
	...
	bool operator==(const string_ncs& other) const
	{
	...
	const bool operator<(const string_ncs& right) const
	{
	...
	void subString(uint32_t begin, uint32_t len, string_ncs& outString)
	{
		uint32_t l = length();
	...
	void append(const string_ncs& other)
	{
		append(other.c_str());
	}
	...
	struct string_hash
	{
		size_t operator()(const string_ncs& _Keyval) const
		{
			string str(_Keyval);
			str.make_lower();
...
typedef string_ncs<8>		string8;
typedef string_ncs<16>		string16;
typedef string_ncs<32>		string32;
typedef string_ncs<64>		string64;
typedef string_ncs<128>	string128;
typedef string_ncs<256>	string256;
typedef string_ncs<512>	string512;
typedef string_ncs<1024>	string1024;
