...
bool CBLPImage::copyMipmapData(uint32_t face, uint32_t level, void* dest, uint32_t pitch, uint32_t width, uint32_t height) const
{
...
bool CBLPImage::loadFile(CMemFile * file)
{
...
bool CBLPImage::fromImageData(const uint8_t* src, const dimension2d& size, ECOLOR_FORMAT format, bool mipmap)
{
	...
	uint8_t* tmpdata = new uint8_t[offset];
