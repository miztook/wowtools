...
void CBlit::convert4BitTo16Bit(const uint8_t* in, uint16_t* out, int32_t width, int32_t height, const uint32_t* palette, int32_t linepad/*=0*/, bool flip/*=false*/)
{
	if (!in || !out || !palette)
	{
		assert(false);
...
void CBlit::convert8BitTo16Bit(const uint8_t* in, uint16_t* out, int32_t width, int32_t height, const uint32_t* palette, int32_t linepad/*=0*/, bool flip/*=false*/)
{
	if (!in || !out || !palette)
	{
		assert(false);
...
void CBlit::convert8BitTo24Bit(const uint8_t* in, uint8_t* out, int32_t width, int32_t height, const uint32_t* palette, int32_t linepad /*= 0*/, bool flip/*=false*/)
{
	if (!in || !out)
		return;

	int32_t lineWidth = 3 * width;
	if (flip)
...
void CBlit::premultiply_A1R5G5B5(void* data, uint32_t width, uint32_t height)
{
...
void CBlit::premultiply_A8R8G8B8(void* data, uint32_t width, uint32_t height)
{
	uint8_t* dst = (uint8_t*)data;
	for (uint32_t y = 0; y < height; ++y)
	{
		...
		{
			float v = dst[x + 3] / 255.0f;
			dst[x] = (uint8_t)(dst[x] * v);
			dst[x + 1] = (uint8_t)(dst[x + 1] * v);
			dst[x + 2] = (uint8_t)(dst[x + 2] * v);
		}
		dst += 4 * width;
	}
}
